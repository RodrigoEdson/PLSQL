create or replace PACKAGE pk_secret_pass AUTHID definer ACCESSIBLE BY ( pk_crypto ) AS
   /**
   * Project:         SAMPLE FOR DBMS_CRYPTO
   * Description:     Contains the functions and procedures to control the keys and IVs 
   *                  on PK_CRYPTO package. This package should have a very restricted 
   *                  access from database users.  The option of being created into a 
   *                  separated schema should be evaluated
   */
  ...
  */
   ...
   * A new Key always will be active for at least for 1s, 
   ...
   * @return                  The ID created to retrieve the P_IV_RAW into the database
   */
   FUNCTION store_iv (

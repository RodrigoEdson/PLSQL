create or replace TYPE BODY to_encrypted_blob_data AS 
   /*========================================================================*/
   ...
   BEGIN
      IF self.encrypted_data IS NOT NULL THEN
         v_string   := 'ENCRYPTED_DATA: '
                     || dbms_lob.getlength(self.encrypted_data)
                     || ' bytes, REFERENCE_DATE: '
                     || TO_CHAR(
            self.reference_date,
   ...
   MEMBER PROCEDURE print_line
      IS
   BEGIN
      dbms_output.put_line('ENCRYPTED_DATA: ' || dbms_lob.getlength(self.encrypted_data)||' bytes');
